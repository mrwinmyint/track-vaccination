...
using CareBaby.Application.Common.Exceptions;
using CareBaby.Application.Common.Security;
using MediatR;
        ...
        if (authorizeAttributes.Any())
        {
            ...
            if (authorizeAttributesWithRoles.Any())
            {
                ...
                foreach (var roles in authorizeAttributesWithRoles.Select(a => a.Roles.Split(',')))
                {
                    ...
                    {
                        var isInRole = await _identityUserRepository.IsInRoleAsync(_currentUserService.UserId.Value, role.Trim());
                        if (isInRole)
                        {
                            authorized = true;
                            break;
            ...
            if (authorizeAttributesWithPolicies.Any())
            {
                ...
                {
                    var authorized = await _identityUserRepository.AuthorizeAsync(_currentUserService.UserId.Value, policy);
